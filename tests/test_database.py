...
class TestDatabase(unittest.TestCase):
    def setUp(self):
        ...
        self.engine = setup_database(self.db_path)
    ...
    def test_insert_data_into_db(self):
        ...
        data_frames = self.create_mock_data()
        final_orders_df = process_data(data_frames)
        insert_data_into_db(self.session, final_orders_df)

        # Verify insertion
        result = self.session.query(Order).count()
        self.assertEqual(result, 2)  # Check if 2 rows are inserted
    ...
    def test_process_data(self):
        ...
        self.assertEqual(final_orders_df.shape[0], 2)  # Check number of rows
        self.assertIn('order_id', final_orders_df.columns)  # Check if 'order_id' is present
