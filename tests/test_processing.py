...
import os
import tempfile
from src.processing import (load_data_from_db, get_customer_spending, get_sales_per_seller, get_product_sales_analysis,
                            get_sales_analysis, save_to_duckdb, analyze_and_load)
...
class TestProcessing(unittest.TestCase):
    ...
    def test_load_data_from_db(self):
        ...
        # Call the function
        result = load_data_from_db(self.db_path, self.table_name)
    ...
    def test_save_to_duckdb(self):
        # Call the function
        save_to_duckdb(self.db_path, self.data, self.table_name)

        # Check if the table exists in DuckDB
        with duckdb.connect(self.db_path) as conn:
            result = conn.execute(f"SELECT * FROM {self.table_name}")
    ...
    def test_analyze_and_load(self):
        ...
        # Call the function
        analyze_and_load(self.db_path, self.table_name)

        # Check if the tables exist in DuckDB
        with duckdb.connect(self.db_path) as conn:
            result1 = conn.execute("SELECT * FROM customers_analysis")
            result2 = conn.execute("SELECT * FROM sellers_analysis")
            result3 = conn.execute("SELECT * FROM products_analysis")
            result4 = conn.execute("SELECT * FROM sales_analysis")
